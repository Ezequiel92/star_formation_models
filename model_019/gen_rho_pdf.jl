...
# ╔═╡ 8d3ace40-f856-11ec-2126-6957c4a13f7b
...
# ╔═╡ 7ca3e649-c877-49a0-af00-352695431d84
...
# ╔═╡ 63528672-039e-4aa9-b183-ce6b1fe2a901
md"## Load the model"

# ╔═╡ f017cef8-1ec4-4f55-b263-a9e9b7421104
model = include_module("./model_019.jl");

# ╔═╡ 1a798694-9970-499b-9cfb-3c585a398d4d
md"# Write density PDF into file"

# ╔═╡ 84066331-aa58-4d7a-bbcb-8a3fb0f15b64
# Save the integrated version of the density PDF function 
# in the file ./gen_files/rho_pdf.txt
begin
	step = (model.DEVIATION[2] - model.DEVIATION[1]) / model.DIVISIONS
	...
	open("./gen_files/rho_pdf.txt", "w") do file
		...
		for i in 1:model.DIVISIONS
			s = first_s + step * (i - 1)
			int_val = quadgk(
				x -> model.FUNCTION(x, model.PARAMS), 
				s - (step / 2), 
				s + (step / 2), 
				order = 10,
			)[1]

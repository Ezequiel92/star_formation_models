/*
 * Make into a shared library in Windows with: gcc -fpic -shared -O3 -march=native -mtune=native interpolation_public.c -o libinterpolationpublic.dll
 * Make into a shared library in Linux with: gcc -fpic -shared -O3 -march=native -mtune=native interpolation_public.c -o libinterpolationpublic.so
 */
...
#define N_COLS 4     // Number of columns in the table
#define N_ROWS 45000 // Number of rows in the table
#define N_DIMS 3     // Number of dimensions of the problem
#define N_VERT 8     // Number of vertices of a n-rectangle = 2^N_DIMS

/* Index to value functions */
static double fun_if(double *idx) { return (0.2 + 0.027586206896551727 * idx[0]); }
static double fun_rh(double *idx) { return (exp(-4.961845129926824 + 0.260936553893533 * idx[1])); }
...
static double inv_fun_rh(double *idx) { return (19.015523336570737 + 8.824310195855288 * log10(idx[1])); }
...
static const int NGRID[] = {30, 50, 30};

...
# ╔═╡ 9f015dce-68bc-11ec-0992-4d23af87f7ff
using DifferentialEquations, PlutoUI, TikzPictures, Unitful, UnitfulAstro, SpecialFunctions, QuadGK, CairoMakie, LaTeXStrings, DataFrames, DataFramesMeta, DelimitedFiles, Trapz, LsqFit, Interpolations, CSV, HDF5, Symbolics, Statistics, Colors

# ╔═╡ c33c0ff8-b3ae-4658-a60d-984d21767c3d
TableOfContents(title="🌌 Parameter analysis", depth=4)

# ╔═╡ e9555d3f-6b93-4a08-9314-85db604cd5c8
...
# ╔═╡ 2de7abcd-ae35-4b25-b64a-cbfb4f4b81c7
md"# Load the model"

# ╔═╡ ccd56dac-122b-4a04-9737-6185fc247601
model = include_module("./description_m019.jl");

# ╔═╡ b9303e6a-f33c-4415-933d-e75e3de4c1bc
md"# Constants"

# ╔═╡ a2ea643b-4fad-4641-a1f3-3fded3fa5bbe
md"#### Integration span"

# ╔═╡ ae6ded20-1cac-426a-9676-3aa46561ee15
begin
	t_start = 0.0u"Myr"
	t_end = 10u"Myr"        
	tspan = [t_start, t_end]
...
# ╔═╡ ecb89610-f655-4493-84ad-8af12e37f24c
md"#### Initial conditions"

# ╔═╡ d5015aac-2588-4ba7-9f3e-e93c767af942
begin
	i₀ = 0.9    
	a₀ = 0.05  
	m₀ = 0.05  
...
# ╔═╡ e7331287-c1c7-4163-ac8f-c10445a1d137
md"#### Reference values for the density PDFs"

# ╔═╡ 67eadbde-b871-458f-933a-0f143521aa2b
param = model.Params(
	b = 0.5,
	Ms = 10.0,
	α = 2.0,
...
# ╔═╡ a3b92a41-5772-43cb-a336-a83624ade949
md"# Analysis"

# ╔═╡ 9e21dae7-6aa8-4b11-b667-a3a967c288ca
md"## Divisions comparison"

# ╔═╡ ec190f70-eace-43a3-990d-8d223c07c224
md"### Logaritmic subdivisons"

# ╔═╡ 6cb7a717-dd3c-4fb1-bb31-83ceb580c27e
let
	divs = [1, 2, 3, 4, 5, 10, 20, 30, 40, 50]
	phase = "stellar"
	
	time, m019 = model.fractions(phase, nothing, param, f₀, tspan)
	time .*= 1000 # Gyr |> Myr
	
	Burkhart2018 = [model.fractions(
		phase,
		model.pBurkhart2018, 
		model.Params(
			b = 0.5,
	...
	iterator = zip(Burkhart2018, divs, colors)
	
	for (Bur18, label, color) in iterator
...
# ╔═╡ e88178d4-3c48-43ab-9c23-749dd489e374
let
	divs = [10, 20, 30, 40, 50, 60, 70, 80]
	phase = "stellar"
	...
	cut = [Bur18[9680] for Bur18 in Burkhart2018]

	lines!(ax, divs, cut, linewidth = 3)
	
	f
end

# ╔═╡ 22c3be12-13cc-43e7-b383-8bc5d18aa107
md"With ~60-70 divisions we have already good convergence."

# ╔═╡ 57f60e76-678f-4b7a-8ea5-d79b060a55bf
md"### Liniar subdivisons"

# ╔═╡ 6e7508ee-ea96-47a1-96a5-26fc6e1596b4
let
	divs = [1, 2, 3, 30, 60, 100, 200, 300, 500]
	phase = "stellar"
	
	time, m019 = model.fractions(
		phase, 
		nothing, 
		param, 
		f₀, 
...
# ╔═╡ e61f4e13-3f87-4a1f-a02d-5270037fc2a9
let
	divs = [1, 2, 3, 30, 60, 300, 500, 700, 900]
	phase = "stellar"
...
# ╔═╡ 5baa9e1c-9fd6-4014-abb8-38479caa781c
md"There is convergence when divisions > 900."

# ╔═╡ 6d73ec17-bc7f-4fac-bff5-6154f8c80de7
md"## Deviation comparison"

# ╔═╡ e5721ff0-92c4-4908-a09d-93464deae597
md"### Logaritmic subdivisons"

# ╔═╡ 6134c50a-27bc-480c-a67f-e5de3c78629f
let
	devs = [1, 2, 3, 4, 5, 6, 7, 8]
	phase = "stellar"
	
	time, _ = model.fractions(phase, nothing, param, f₀, tspan)
	time .*= 1000 # Gyr |> Myr
	...
	iterator = zip(Burkhart2018, devs, colors)
	
	for (y_axis, label, color) in iterator
		lines!(ax, time, y_axis; linewidth = 3, label = "deviation = $label", color)
	end
	
	axislegend(;position = :lt, labelsize = 14, nbanks = 2)
	
	f
end

# ╔═╡ 210fe0ef-77ba-4f31-aea3-009fa362ad29
let
	devs = [4, 5, 6, 7, 8, 9, 10, 11]
	phase = "stellar"
	...
	lines!(ax, devs, cut, linewidth = 3)
	
	f
end

# ╔═╡ 36674648-f37e-425d-b9df-a6a3b009ef4a
md"With a deviation of ~7 we have already good convergence."

# ╔═╡ fabb2a42-3242-40c9-951a-723760adb92b
md"# Convergence"

# ╔═╡ 3622dcd6-d92f-47db-896a-d07b0a6c58ed
let
	phase = "stellar"
	
	time, Bur2018_a = model.fractions(
		phase,
		model.pBurkhart2018, 
		model.Params(
			b = 0.5,
	...
	lines!(
		...
		linewidth = 3, 
		color = :red,
		label = "divisions = 100.0 - deviation = 10.0",
	)
	lines!(
		ax, 
		time, Bur2018_b, 
		...
		color = :blue,
		linestyle = :dash,
		label = "divisions = 10.0 - deviation = 6.0",
	)

	diff = abs.(Bur2018_a - Bur2018_b) ./ Bur2018_a
	ax2 = Axis(
		f[2,1], 
		xlabel = "t [Myr]", 
		ylabel = "relative difference (x10^3)", 
		xlabelsize = 15,
		ylabelsize = 15,
		xticklabelsize = 15,
		yticklabelsize = 15,
...
# ╔═╡ 26a637e0-cf86-4ff6-b0f6-904b8fd86c19
md"# Dependance with the constant parameters"

# ╔═╡ f61303ea-c3c7-4864-b06b-aadd695de7b7
md"## Power law slope (α)"

# ╔═╡ 445fe1a9-2151-4c4b-b23f-e0a3b09318cf
let
	alphas = [i for i in 1.5:0.1:3]
	phase = "stellar"
	...
	ax = Axis(
		f[1,1], 
		xlabel = "α", 
		ylabel = "$phase fraction - 2.93x10^-5", 
		xlabelsize = 20,
		ylabelsize = 20,
		xticklabelsize = 20,
		yticklabelsize = 20,
	...
	cut = [Bur18[9680] - 2.93*(10^-5) for Bur18 in Burkhart2018]

	lines!(ax, alphas, cut, linewidth = 3)
	
	f
end

# ╔═╡ c2566009-b22b-4cb2-8cf8-e97acc663a17
md"## Dimensionless turbulent forcing parameter (b)"

# ╔═╡ 25828d50-1a92-43bd-8536-d9827fcac014
let
	bs = [i for i in 0.1:0.1:1]
	phase = "stellar"
	...
	lines!(ax, bs, cut, linewidth = 3)
	
	f
end

# ╔═╡ c842a484-381d-4a51-ada4-7520a6642dd1
md"## Mach number (Ms)"

# ╔═╡ bfec3af7-1efc-49e5-aa0c-e27985d3a366
let
	machs = [i for i in 1:10:100]
	phase = "stellar"
	...
	lines!(ax, machs, cut, linewidth = 3)
...
git-tree-sha1 = "557aba64d657fe0967e1bdd128e7efcae6504fb8"
...
git-tree-sha1 = "efabcabc06030e2b8f072f1b47bc823b6bb3c619"
...
git-tree-sha1 = "785766c07cd4e7e7c2989c27f5b3444e969b1733"
...
git-tree-sha1 = "4e2115de913d985dd622cabd2694c3e0ddaf0e08"

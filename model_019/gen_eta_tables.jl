...
# ╔═╡ f4804410-f91c-11ec-30d3-196bf54ebe23
...
# ╔═╡ 4dff3b54-db7b-4a40-887c-bec6629b3dc5
...
# ╔═╡ ab8c1623-5491-46a3-a0df-6d1d1b33096c
md"# Load model"

# ╔═╡ 883f57b0-129e-4079-b648-321cbe6b9e16
model = include_module("./model_019.jl");

# ╔═╡ 06084855-2b1b-4a1a-8872-c0540a4f67a3
md"# Save the $\eta$ values in text files"

# ╔═╡ e64f6f58-f6fd-454c-a1bf-faf96efe1949
# Each file is named after the metallicity and is 
# stored within the folders ./gen_files/eta_tables/`IMF`/
for (name, imf) in zip(model.col_names[2:end], model.Q_imfs)
	for z in model.Q_metals
		dir = mkpath("./gen_files/eta_tables/$name/")
		...
		for index_age in 5:size(q_df, 1)
			...
			eta_data = vcat(
				...
				[
					...
					ustrip(Unitful.NoUnits, trapz(ages, QH) * model.f_ion);;
					ustrip(Unitful.NoUnits, trapz(ages, QHII) * model.f_diss)
				],
			)
		end

...
# ╔═╡ a58c1320-59c6-11ec-3e22-d913897814bb
using DifferentialEquations, PlutoUI, TikzPictures, Unitful, UnitfulAstro, SpecialFunctions, QuadGK, CairoMakie, LaTeXStrings, DataFrames, DataFramesMeta, DelimitedFiles, Trapz, LsqFit, Interpolations, CSV, HDF5

# ╔═╡ ef184df7-28b1-416e-8c21-ae534325817f
TableOfContents(title="🔢 Density PDF", depth=4)

# ╔═╡ 084ba8f2-5ad9-4160-909e-b13374120dfc
...
# ╔═╡ 67410f1f-b13d-41a4-a6c3-920bac7050b7
md"# Load the model"

# ╔═╡ 9fc9b135-ce13-4170-9e08-50960c8618ad
model = include_module("../description_m019.jl");

# ╔═╡ ea6c8de8-2998-4276-8d14-49155cee2eb7
md"# Density PDF functions"

# ╔═╡ 67cccd80-eabd-414e-be02-8941ec8a523a
params = model.Params(
	b = 0.5,
	Ms = 10.0,
	α = 2.0,
	divisions = 50,       
	deviation = (-8, 4.5),
	ρ₀ = ustrip(u"Msun * pc^-3", 0.9u"mp * cm^-3"),
);

# ╔═╡ 91f294ea-b57d-4378-b1de-1517b66a2f6f
s0 = -0.5 * log(1 + params.b^2 * params.Ms^2)

# ╔═╡ 352b200c-936c-437e-8df0-c032f5327927
md"### Burkhart et al. (2018)"

# ╔═╡ 87eceb55-0872-4219-9b9c-0b03f98c0bbb
md"#### Normalization"

# ╔═╡ 953b558c-75ad-4693-8a0d-2fbbfb216492
let
	func = x -> model.pBurkhart2018(x, params)
	quadgk(func, -20, 20)[1]
end

# ╔═╡ 4bf836e8-b7f2-42c5-b18d-f44749fbaf1d
md"#### Plot"

# ╔═╡ db56b86b-de04-4a13-b7e6-e1fce269307d
let
	x_axis = collect(-8:0.01:6)
	y_axis = [model.pBurkhart2018(x_ax, params) for x_ax in x_axis]
...
# ╔═╡ a6152607-48a8-446e-82ed-c38d0ca7a7c5
md"### Krumholz et al. (2005)"

# ╔═╡ 0af46c1f-7ed6-4102-ae0d-873c4468863b
md"#### Normalization"

# ╔═╡ 51e352c7-ff39-4310-bbeb-e10bc4642ed3
let
	func = x -> model.pKrumholz2005(x, params)
	quadgk(func, -20, 20)[1]
end

# ╔═╡ 8f64d7b1-8f66-4c5b-ba8d-63707259f18b
md"#### Plot"

# ╔═╡ 77bd8742-8c2f-47ed-8bda-95c69eb588bd
let
	x_axis = collect(-8:0.01:6)
	y_axis = [model.pKrumholz2005(x_ax, params) for x_ax in x_axis]
...
# ╔═╡ 64b1c4f9-4c3b-48e1-b133-3b73f2f81356
md"### Plot of both density functions"

# ╔═╡ 24319ed4-2fa8-4270-b2aa-7b0e73dd1c61
let
	x_axis = collect(-8:0.01:6)
	Krumholz2005 = [model.pKrumholz2005(x_ax, params) for x_ax in x_axis]
	Burkhart2018 = [model.pBurkhart2018(x_ax, params) for x_ax in x_axis]
...
# ╔═╡ c0891f0f-334d-4ca6-a605-9998181703b0
md"# Integration span"

# ╔═╡ 537a53df-392a-426c-929c-7d38521b1ca7
linear_mass_fraction, linear_points = model.mass_fraction(
	model.pKrumholz2005, 
	params,
	false,
);

# ╔═╡ 6076a707-6a7e-42b3-9fd5-dce9a895e092
linear_points

# ╔═╡ 8251d840-b29c-43f1-b4e2-c0b705b3accc
sum(linear_mass_fraction)

# ╔═╡ 51528787-cb9d-4130-8116-670d255591fc
log_mass_fraction, log_points = model.mass_fraction(
	model.pKrumholz2005, 
	params,
	true,
);

# ╔═╡ 744af81e-eb54-4da4-81e4-39fafdf7f90e
log_points

# ╔═╡ c5c3c909-8af3-4bc8-9a27-eafda9c16ef1
sum(log_mass_fraction)

# ╔═╡ 8db9a207-b352-43de-944a-9d74a5d36cb1
let
	f = Figure()
	ax = Axis(
		f[1,1], 
		...
		xticklabelsize = 20,
		yticklabelsize = 20,
		title = "Logaritmic divisions",
	)
	x_axis = collect(-10:0.01:10)
	y_axis = [model.pKrumholz2005(x_ax, params) for x_ax in x_axis]
	
	lines!(ax, x_axis, y_axis, linewidth = 3, color = :orange)
	vlines!(ax, log_points)
	vlines!(ax, s0, color=:red)

	f	
end

# ╔═╡ c1bfd81d-09ed-4b55-a4e3-56c6b24a21df
let
	f = Figure()
	ax = Axis(
		f[1,1], 
		xlabel = L"f = \rho / \rho_0", 
		ylabel = L"\mathrm{Probability}",
		xlabelsize = 25,
		ylabelsize = 25,
		xticklabelsize = 20,
		yticklabelsize = 20,
		xscale = log10,
		title = "Linear divisions",
	)
	x_axis = collect(-10:0.01:10)
	y_axis = [model.pKrumholz2005(x_ax, params) for x_ax in x_axis]
	
	lines!(ax, exp.(x_axis), y_axis, linewidth = 3, color = :orange)
	vlines!(ax, linear_points)
	vlines!(ax, exp.(s0), color=:red)

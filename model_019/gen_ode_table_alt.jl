...
using CSV, CairoMakie, DataFrames, DataFramesMeta, DelimitedFiles, DifferentialEquations, HDF5, Interpolations, LaTeXStrings, PlutoUI, QuadGK, SpecialFunctions, Statistics, Symbolics, TikzPictures, Trapz, Unitful, UnitfulAstro, Printf, Measurements, Colors
...
model = include_module("./model_019_alt.jl");
...
begin
	...
	const TABLE = "./gen_files/ode_tables/ode_table_public_alt.txt"
	const TABLE_RHO = "./gen_files/ode_tables/ode_table_rho_pdf_public_alt.txt"
	const KW_ARGS = (dense=false, force_dtmin=true, reltol=1e-10, maxiters=1e7)
	const ALG = TRBDF2()
end;
...
begin
	...
	open("./gen_files/functions/functions_public_alt.txt", "w") do file
	...
	for idx[1] in 0:(N[1] - 1)
		...
		for idx[2] in 0:(N[2] - 1)
			...
			for idx[3] in 0:(N[3] - 1)
				...
				# Write to file
				fmt = Printf.Format(
					"%.$(PRES)f %.$(PRES)f %.$(PRES)f %.$(SOL_PRES)f\n",
				)
				open(path, "a") do file
...
begin
	n_nan = write_table(TABLE, false)
	n_nan_rho = write_table(TABLE_RHO, true)
	if n_nan > 0

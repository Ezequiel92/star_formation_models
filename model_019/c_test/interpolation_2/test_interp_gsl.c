    ...
    for (size_t i = 0; i < N_VERT; ++i)
    {
        ...
        for (size_t j = 0; j < DIMENSIONS; ++j)
        {
            if (val_low[j] != val_high[j])
            {
                ...
                printf("vertices[i][j] = %f\n", vertices[i][j]);
                printf("val_low[j] = %f\n", val_low[j]);
                printf("vertices[i][j] == val_low[j] = %d\n", vertices[i][j] == val_low[j]);
        ...
        printf("vertices[%d] = %.12f %.12f %.12f %.12f\n", i, vertices[i][0], vertices[i][1], vertices[i][2], vertices[i][3]);
        printf("vertices[%d][%d] = %.12f\n", i, DIMENSIONS, vertices[i][DIMENSIONS]);
        printf("degen_factor = %d\n", degen_factor);
...
int main()
{
    ...
    size_t cols, rows;
    double **data_table;
    char paths[] = "../../data/ode_table_rho_pdf.txt";
    data_table = read_ftable(paths, &rows, &cols);

    double point[] = {0.10080584897919226, 0.004301609809193359, 0.005960465868844676, 0.0020077658840191932};

    double result = interpolate(data_table, &rows, &cols, point);
    printf("result: %.12f\n", result);

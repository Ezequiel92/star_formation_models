...
using OrdinaryDiffEq, Plots, PlutoUI, Symbolics
...
# Values of α
const ALPHA = string.(collect(0.1:0.1:1.0));
...
md"# Comparison of different values of α for model 16"
...
begin 
	paths = ["./model_016_" * i * ".jl" for i in ALPHA]
	models = [include_module(path) for path in paths]
end;

# ╔═╡ 224211d9-c03d-472f-bb02-575f64211735
solutions = [
	integrate_model(model.system!, initial_condition, tspan, parameters) 
	for model in models
];

# ╔═╡ c1537ea5-7c50-4272-a358-50ea0da2e2f9
begin
	inter_sols = [solution(times) for solution in solutions]
	safe_t = [
		replace(x -> x <= 0 ? NaN : x, inter_sol.t) 
		for inter_sol in inter_sols
	]
	safe_s = [
		replace(x -> x <= 0 ? NaN : x, inter_sol[5, :]) 
		for inter_sol in inter_sols
	]
end;
...
plot(
	...
	linewidth = 2, 
	linestyle = :auto,
	color = [:red :blue :green :pink :purple :orange :black :cyan :brown :magenta],
)

# ╔═╡ 16cac8ac-aebf-4b7c-9f8f-6cf0585fa241
# Stellar fraction at a fix time (1e-2, 1e-3 and 1e-4 Gyr) for every α
begin
	fix_t_2 = [solutions[i](1e-2)[5] for i in 1:length(ALPHA[1:(end-1)])]
	fix_t_3 = [solutions[i](1e-3)[5] for i in 1:length(ALPHA[1:(end-1)])]
	fix_t_4 = [solutions[i](1e-4)[5] for i in 1:length(ALPHA[1:(end-1)])]
end;

# ╔═╡ 167821c9-a084-4014-a6e9-bee3e610e9db
plot(
	ALPHA[1:(end-1)], 
	[fix_t_2 fix_t_3 fix_t_4], 
	xaxis = "α", 
	yaxis = ("stellar fraction", :log),
	label = ["t = 1e-2" "t = 1e-3" "t = 1e-4"],

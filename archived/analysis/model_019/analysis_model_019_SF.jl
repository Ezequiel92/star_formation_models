...
TableOfContents(title="🌌 Analysis of model 019 - SF", depth=4)

# ╔═╡ 181ee622-4249-447d-b4ab-ba503208ef9e
...
# ╔═╡ f422341b-deaa-42c9-ba5e-3b127329399e
md"# Load the model"

# ╔═╡ 3b086da4-e879-4592-9460-3c0fe8d02683
model = include_module("../description_m019.jl");

# ╔═╡ 929feae3-9130-4f2d-ba38-0f61a3347570
md"# Constants"

# ╔═╡ 017d54e5-dff6-4275-ae11-a1ce787b7d7d
# Parameters for the density PDF
Base.@kwdef struct Params
	...
	# Mach number
	Ms::Float64
	# Maximum value of s = log(ρ/ρ₀)
	max_deviation::Float64
	# Number of subdivisions of s ∈ [-max_deviation, max_deviation]
	divisions::Int64
	# Total initial mean density ρ₀ = tot0 [Mₒ pc^(-3)] = n [cm^-3] / P
	ρ₀::Float64             	
end;
...
# ╔═╡ 6a8044f1-8d77-4429-a91c-d1ffb27caeb1
md"#### Reference values for the density PDFs"

# ╔═╡ 63637a5c-a2fc-418c-9242-e8d4823cf5d2
param = Params(
	b = 0.5,
	Ms = 10.0,
	α = 2.0,
...
# ╔═╡ dae9bcf4-da09-4d45-9955-e7cdde577fa3
function deltas(v)
	return prepend!([v[i] - v[i - 1] for i in 2:length(v)], 0.0)
end;

# ╔═╡ 35fd187e-5d0a-44ed-8074-c46d339e0999
function star_fraction(
		ρ_PDF, 
		params; 
		sfr = false, 
		init_cond = f₀, 
	...
	if timesteps === nothing
		...
		times = [10^i for i in log_t_start:t_step:log_t_end]
	else
		times = timesteps
	end
	...
	if ρ_PDF === nothing
		...
		################################################
		# Standard model 019, not using the density PDF
		################################################
		
		sol = model.integrate_model(
			init_cond, 
			tspan,
			[params.ρ₀, init_cond[1] + init_cond[2] + init_cond[3]],
		)
		inter_sol = sol(times)
		stellar_fraction = replace(x -> x <= 0 ? NaN : x, inter_sol[5, :])
		
	else
		
		##################################
		# Model 019 using the density PDF 
		##################################
		
		# Density step
    	ρ_step = (2 * params.max_deviation) / params.divisions
		# First value of s = log(ρ/ρ₀)
    	s0 = (ρ_step / 2.0) - params.max_deviation
		
		stellar_fraction = zeros(Float64, length(times))
		for i in 1:params.divisions
			
			# s = log(ρ/ρ₀)
			s = s0 + ρ_step * (i - 1) 
			# Density PDF
			func = x -> ρ_PDF(x, params)
			# Fraction of each particle mass within the density range
        	mass_fraction = quadgk(func, s - (ρ_step / 2.0), s + (ρ_step / 2.0))[1]
			
        	sol = model.integrate_model(
				init_cond, 
				tspan, 
				[params.ρ₀ * exp(s), init_cond[1] + init_cond[2] + init_cond[3]],
			)
			inter_sol = sol(times)
			stellar_fraction .+= replace(
				x -> x <= 0 ? NaN : x, 
				inter_sol[5, :],
			) .* mass_fraction
	...
	if sfr
		...
		stellar_ρ = stellar_fraction .* params.ρ₀
		Δsρ = deltas(stellar_ρ)
		
		return times, Δsρ ./ deltas(times)
		
	else
		
		return times, stellar_fraction
...
# ╔═╡ 98aac2d0-ffa7-44de-a1be-c205e7a32d43
function pBurkhart2018(s, params)
	...
	σs = sqrt(log(1 + b^2 * Ms^2))
	s0 = -0.5 * σs^2
	st = (α - 0.5) * σs^2
	C = exp((α - 1) * 0.5 * α * σs^2) / (sqrt(2π) * σs)
	N = 1 / ((C * exp(-α * st)) / α + 0.5 + 0.5 * erf((2 * st + σs^2) / (2 * sqrt(2) * σs)))
	
	if s < st
		return (N / (sqrt(2π) * σs)) * exp(-((s - s0)^2) / (2 * σs^2))
	else
		return N * C * exp(-α * s)
	end
...
let
	...
	y_axis = [pBurkhart2018(x_ax, param) for x_ax in x_axis]
...
# ╔═╡ 7c38e007-74fd-4477-a13f-94ad0c3da53b
function pKrumholz2005(s, params)
	...
	return exp(-((s - s0)^2) / (2 * σs^2)) / (sqrt(2π) * σs)
...
let
	x_axis = collect(-8:0.01:6)
	y_axis = [pKrumholz2005(x_ax, param) for x_ax in x_axis]
...
let
	x_axis = collect(-8:0.01:6)
	Krumholz2005 = [pKrumholz2005(x_ax, param) for x_ax in x_axis]
	Burkhart2018 = [pBurkhart2018(x_ax, param) for x_ax in x_axis]
...
let
	time, m019 = star_fraction(nothing, param)
	time .*= 1000 # Gyr |> Myr
	_, Krumholz2005 = star_fraction(pKrumholz2005, param)
	_, Burkhart2018 = star_fraction(pBurkhart2018, param)	
	...
	lines!(ax, time, m019, linewidth = 3, label = "ρ₀ = $n₀")
	lines!(ax, time, Krumholz2005, linewidth = 3, label = "Krumholz2005")
	lines!(
		ax, 
		time, Burkhart2018, 
		linewidth = 3, 
...
# ╔═╡ 552052a4-3b3a-4393-8603-0597aabc0c94
let
	time, m019 = star_fraction(nothing, param, sfr = true)
	time .*= 1000 # Gyr |> Myr
	_, Krumholz2005 = star_fraction(pKrumholz2005, param, sfr = true)
	_, Burkhart2018 = star_fraction(pBurkhart2018, param, sfr = true)	

	f = Figure()
	ax = Axis(
		f[1,1], 
		xlabel = "t [Myr]", 
		ylabel = "SFRρ [$(string(UnitfulAstro.Msun / UnitfulAstro.pc^3 / UnitfulAstro.Gyr))]", 
		yscale = log10,
		xlabelsize = 20,
		ylabelsize = 20,
		xticklabelsize = 20,
...
md"It seems like with `max_deviation = 4` the solution converges, so resonable values may be `max_deviation = 4` and `divisions = 8.0`."
...
let
	...
	for ((idx, time), c) in zip(enumerate(T), colors)
		scatterlines!(
			ax, 
			Z, 
			Kr2005[idx, :], 
...
# ╔═╡ 43761c08-54e4-4be0-9526-2ef74e370e81
md"## Initial conditions comparison"

# ╔═╡ 32172a5f-b2b2-4cc4-b186-2d48b9a50a73
let	
	af = [i for i in 0.01:0.01:0.09] 
	mf = [1- a0 - f₀[1] for a0 in af]
	conditions = [[f₀[1], a0, m0, f₀[4], f₀[5]] for (a0, m0) in zip(af, mf)]
	...
	for (y_axis, a0, m0) in zip(Krumholz2005, af, mf)
		lines!(
			ax, 
			time, y_axis; 
			linewidth = 3, 
			label = "af₀ = $a0 - mf₀ = $(round(m0; sigdigits = 2))",
		)
	end
...
# ╔═╡ 2236df1b-4fae-42a8-9176-003daa7234f1
let
 	af = [i for i in 0.01:0.01:0.09]  
	mf = [1- a0 - f₀[1] for a0 in af]
	conditions = [[f₀[1], a0, m0, f₀[4], f₀[5]] for (a0, m0) in zip(af, mf)]
	...
	ax = Axis(
		f[1,1], 
		xlabel = "initial atomic fraction", 
		ylabel = "stellar fraction", 
		xlabelsize = 20,
		ylabelsize = 20,
		xticklabelsize = 20,
...
# ╔═╡ d81fa677-b54e-463b-b8e1-8c28547248b7
let
 	af = [i for i in 0.01:0.01:0.09]  
	mf = [1- a0 - f₀[1] for a0 in af]
	conditions = [[f₀[1], a0, m0, f₀[4], f₀[5]] for (a0, m0) in zip(af, mf)]
	...
	ax = Axis(
		f[1,1], 
		xlabel = "initial molecular fraction", 
		ylabel = "stellar fraction", 
		xlabelsize = 20,
		ylabelsize = 20,
		xticklabelsize = 20,

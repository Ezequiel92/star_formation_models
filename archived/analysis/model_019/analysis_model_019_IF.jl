...
# ╔═╡ f9fdfba8-c7a8-434c-a0ca-252c327fe401
using DifferentialEquations, PlutoUI, TikzPictures , Unitful, UnitfulAstro, CairoMakie, SpecialFunctions, QuadGK, Colors, Printf

# ╔═╡ 6b9f7e3a-6ecb-45a7-8011-95ef498d5a67
TableOfContents(title="🌌 Analysis of model 019 - IF", depth=4)

# ╔═╡ 9d89e4c5-56c1-4dc7-af54-ebbd6a8f162f
...
# ╔═╡ cb0d8533-0a85-4db6-a262-8a0ed278f2e0
md"# Load the model"

# ╔═╡ 8e2802cf-cc7d-436d-9f17-9729eaa5beeb
model = include_module("../description_m019.jl");

# ╔═╡ 929feae3-9130-4f2d-ba38-0f61a3347570
md"# Constants"

# ╔═╡ 145485cb-83af-429a-8ba4-69aa59f6d34c
...
Base.@kwdef struct Params
	...
	divisions::Int64
	# Total initial mean density ρ₀ [Mₒ pc^(-3)]
	ρ₀::Float64        
end;

# ╔═╡ f201c5e9-84c7-4b03-bdfa-c060fdafea05
md"#### Integration span"

# ╔═╡ 093f83b6-d78d-46bc-9046-cb402d3b9e5a
begin
	t_start = 0.0    # [Gyr]
	t_end = 0.01     # [Gyr]           
	tspan = (t_start, t_end)
...
# ╔═╡ b1df8305-3ca3-43d0-ab7f-d1be64f4917f
...
function ionized_fraction(
	...
	if ρ_PDF === nothing
		...
		inter_sol = sol(times)
		ionized_fraction = replace(x -> x <= 0 ? NaN : x, inter_sol[1, :])
		...
		ionized_fraction = zeros(Float64, length(times))
...
let
	time, m019 = ionized_fraction(nothing, param)
	time .*= 1000 # Gyr |> Myr
	_, Krumholz2005 = ionized_fraction(pKrumholz2005, param)
	_, Burkhart2018 = ionized_fraction(pBurkhart2018, param)	
...
let
	...
	axislegend(;position = :rt, labelsize = 12, rowgap = 1, nbanks = 2)
...
let
	...
	axislegend(;position = :rt, labelsize = 14, nbanks = 2)

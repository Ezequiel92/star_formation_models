...
# ╔═╡ 3f1ca543-33e6-41f4-b583-c79cdaa60acb
md"### No stars"

# ╔═╡ f312d523-b96c-4363-b23e-27abf3efe927
begin
	s0_NS = 0.0
	init_cond_NS = [i0, a0, m0, z0, s0_NS]
end;

# ╔═╡ ed8334de-484a-49b4-9cb0-11b3515fb601
md"### Half stars"

# ╔═╡ df4b3060-ac32-47bd-92bf-199b541e04d9
begin
	s0_HS = 0.5
	init_cond_HS = [i0, a0, m0, z0, s0_HS]
end;
...
# ╔═╡ 2af4d85f-4544-4c0c-9d9b-21422ee235d3
md"### Fix Zsn and variable Zeff"

# ╔═╡ 8124d959-a4c5-445a-87cc-6da62295e012
begin
	zsn = 0.09
	values_Zeff = [Zsun * 10^i for i in -4:0.1:4]
	Z_params_eff = [[z_eff, zsn] for z_eff in values_Zeff]
end;

# ╔═╡ dc0dd48c-5e93-4d93-8f07-faf967b6ca75
md"### Fix Zeff and variable Zsn "

# ╔═╡ 0cb71df3-f7e1-4691-8f86-622a365d5cc9
begin
	zeff = 1e-3 * Zsun
	values_Zsn = [9 * 10^i for i in -3:0.05:3]
	Z_params_sn = [[zeff, z_sn] for z_sn in values_Zsn]
end;

# ╔═╡ 1726d9b8-36eb-48c4-bc94-cc311c414079
md"## Integration"

# ╔═╡ f0712add-070e-499e-8d8b-4df632e84c2e
md"### No stars + fix Zsn and variable Zeff"

# ╔═╡ ac72cecb-6d70-4245-9656-edb7e4e6d4b7
md"### No stars + fix Zeff and variable Zsn"

# ╔═╡ a8d910f4-bdb6-47e4-b906-7887f9ac2cb0
md"### Half stars + fix Zsn and variable Zeff"

# ╔═╡ 2fee26a6-c814-46bb-a50c-326e85940fd2
md"### Half stars + fix Zeff and variable Zsn"
...
function integrate_model(y₀, tspan, params)
	...
	return sol(tspan[2])[5]
end;

# ╔═╡ b2441d8f-30e9-466f-be6c-4e867d710343
begin
	params_01 = [vcat(base_params, z_params) for z_params in Z_params_eff]
	star_frac_01 = [
		integrate_model(init_cond_NS, tspan, params)
		for params in params_01
	]
end;
...
plot(
	...
	star_frac_01, 
	xaxis = ("Zeff", :log),
	yaxis = "stellar fraction after 50Myr",
	title = "No inital stars",
)

# ╔═╡ 68676b70-088a-402d-96eb-f371f579f071
begin
	params_02 = [vcat(base_params, z_params) for z_params in Z_params_sn]
	star_frac_02 = [
		integrate_model(init_cond_NS, tspan, params)
		for params in params_02
	]
end;

# ╔═╡ f6f1c570-a6cd-4b73-9b37-5d858a85168d
plot(
	values_Zsn, 
	round.(star_frac_02, sigdigits = 4),
	xaxis = ("Zsn", :log),
	yaxis = "stellar fraction after 50Myr",
	yrange = (0.25, 0.4),
	title = "No inital stars",
...
# ╔═╡ 9c0ea4aa-0f28-4833-9545-2a583900b47d
plot(
	values_Zsn, 
	round.(star_frac_02, sigdigits = 4), 
	xaxis = ("Zsn", :log),
	yaxis = "stellar fraction after 50Myr",
	yrange = (0.25, 0.4),
	title = "Half inital stars",
)

# ╔═╡ a215e70f-b34d-4d3d-a6ee-2b733125aa38
begin
	params_03 = [vcat(base_params, z_params) for z_params in Z_params_eff]
	star_frac_03 = [
		integrate_model(init_cond_HS, tspan, params)
		for params in params_03
	]
end;

# ╔═╡ e8efbf76-8afe-47c5-aae2-f36f436c5d82
plot(
	values_Zeff, 
	star_frac_03, 
	xaxis = ("Zeff", :log),
...
# ╔═╡ 8d24013c-eb2d-4752-925a-bbf1b1bbf5b3
begin
	params_04 = [vcat(base_params, z_params) for z_params in Z_params_sn]
	star_frac_04 = [
		integrate_model(init_cond_HS, tspan, params)
		for params in params_04
	]
end;

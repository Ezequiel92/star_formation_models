...
using OrdinaryDiffEq, Plots, PlutoUI, LaTeXStrings, Symbolics
...
# ╔═╡ 7c85805b-b475-4a3f-b633-27d818aa246e
md"""
* 00: Model 16 unmodified.
* R: Couplin (with a factor of 'R') of the stellar equation with the ionized gas equation.
* MA: ψ = (m + a) / τS instead of simply ψ = m / τS.
* NV: Variable number density instead of constant number density.
* Z: Coupled metallicity (Z = z instead of Z = z0).
* NV+R+Z: NV, R and Z combined.
* NV+R+Z+MA: NV, R, Z and MA combined.
"""

# ╔═╡ c6bdbdc5-f969-4420-9114-6d4044a138b9
# Different version of Model 016
const MODS = string.(["00", "R", "MA", "NV", "Z", "NV+R+Z", "NV+R+Z+MA"]);
...
begin
	...
	parameters = [parameters_A, parameters_B, parameters_C, parameters_D]
end;

# ╔═╡ d54ca6b9-f917-48e3-b4a8-119adec185cc
md"# Comparison of different modifications for model 16"
...
# ╔═╡ 7aed1267-12c3-4bf9-be6c-0da9700fd00b
...
# Run the models inside a module 
begin 
	paths = ["./model_016_" * i * ".jl" for i in MODS]
	models = [include_module(path) for path in paths]
end;

# ╔═╡ 224211d9-c03d-472f-bb02-575f64211735
solutions = [[
	integrate_model(model.system!, initial_condition, tspan, parameter) 
	for model in models
] for parameter in parameters];

# ╔═╡ c1537ea5-7c50-4272-a358-50ea0da2e2f9
begin
	discrete_sols = [[solution(times) for solution in sols] for sols in solutions]
	safe_t = [[
		replace(x -> x <= 0 ? NaN : x, discrete_sol.t) 
		for discrete_sol in disc_sols
	] for disc_sols in discrete_sols]
	safe_s = [[
...
plot(
	...
	title = L"\mathrm{n} = 4\,\mathrm{cm^{-3}}",
	label = hcat(MODS...),
	legend = :bottomright,
	legendfontsize = 10,
	guidefont = 16, 
	...
	linewidth = 2, 
	linestyle = :auto,
	color = [:red :blue :green :orange :pink :purple :cyan],
)

# ╔═╡ e02082f9-c2b6-4e1e-b7bc-288119c5206d
plot(
	hcat(safe_t[2]...), 
	hcat(safe_s[2]...), 
	xaxis = ("t / Gyr", :log), 
	yaxis = ("stellar fraction", :log),
	title = L"\mathrm{n} = 40\,\mathrm{cm^{-3}}",
	label = hcat(MODS...),
	legend = :bottomright,
	legendfontsize = 10,
	guidefont = 16, 
...
# ╔═╡ 2a1ff6c4-5e76-4e98-af69-50ddede4a405
plot(
	hcat(safe_t[3]...), 
	hcat(safe_s[3]...), 
	xaxis = ("t / Gyr", :log), 
	yaxis = ("stellar fraction", :log),
	title = L"\mathrm{n} = 400\,\mathrm{cm^{-3}}",
	label = hcat(MODS...),
	legend = :bottomright,
	legendfontsize = 10,
	guidefont = 16, 
...
# ╔═╡ 2bed8a1b-6126-4589-8844-a3cdf889d1f5
plot(
	hcat(safe_t[4]...), 
	hcat(safe_s[4]...), 
	xaxis = ("t / Gyr", :log), 
	yaxis = ("stellar fraction", :log),
	title = L"\mathrm{n} = 4000\,\mathrm{cm^{-3}}",
	label = hcat(MODS...),
	legend = :bottomright,
	legendfontsize = 10,
	guidefont = 16, 

...
# ╔═╡ 95eb8e5b-b726-4d6f-8f7d-1e10bd016d52
using OrdinaryDiffEq, Plots, PlutoUI, Symbolics, LaTeXStrings, PGFPlotsX

# ╔═╡ f8edca81-5b7e-40c4-9570-94dc670ead1e
TableOfContents(title="🌌 Table of contents", depth=4)

# ╔═╡ a6b4807c-2d32-4f44-940c-aa3a9fdf4305
md"##### Initializing packages"

# ╔═╡ 6d228025-a5b4-4b25-be41-c88a6df1e216
md"# Models"

# ╔═╡ 28841df2-df28-4b0f-b170-b9cfba679254
# Path to every model notebook
begin
	first = [
		"../models/model_00" * i * "/notebook_00" * i * ".jl" 
		for i in string.(1:9)
	]
	second = [
		"../models/model_0" * i * "/notebook_0" * i * ".jl" 
		for i in string.(10:18)
	]
	paths = vcat(first, second)
end;

# ╔═╡ c6bdbdc5-f969-4420-9114-6d4044a138b9
# Which models to load
begin
	const LOAD_MODELS_A = [1, 2, 3, 4, 5, 6, 15, 16, 18] 	# Dimensionless
	const LOAD_MODELS_B = [7, 9, 10, 11, 12, 13]			# Area density
	const LOAD_MODELS_C = [8, 17]							# Volume density
end;

# ╔═╡ 5b1bd036-317c-4328-953e-2d23e2de6617
md"## Integration span"

# ╔═╡ b24f09af-30e1-49d0-a0c3-b4caa30c5702
begin
	t_start = 0.0    # [Gyr]
	t_end = 1.0      # [Gyr]           
	tspan = (t_start, t_end)
...
# ╔═╡ 7ad2de09-8655-4c83-bbad-f0738b67c184
md"## Initial conditions"

# ╔═╡ 4855e70f-77fd-4b59-9116-8b8966a56351
begin

    # From a density of 10 in internal units (details in constants_calculation.jl)
	
	n_density = 4046       # n [cm^(-3)]
	density_2 = 6e4        # ρ [Mₒ pc^(-2)]
	density_3 = 100        # ρ [Mₒ pc^(-3)]
    ...
    # Models 001, 002, 003, 004, 005, 006, 014, 015, 016, 018
	init_cond_A = [i₀, a₀, m₀, z₀, s₀]
	
	# Models 007, 009, 010, 011, 012, 013
	init_cond_B = density_2 .* [i₀, a₀, m₀, z₀, s₀]

    # Models 008, 017
	init_cond_C = density_3 .* [i₀, a₀, m₀, z₀, s₀]
	
	initial_conditions = [
		init_cond_A,	# Model 001
		init_cond_A,	# Model 002
		init_cond_A,	# Model 003
		init_cond_A,	# Model 004
		init_cond_A,	# Model 005
		init_cond_A,	# Model 006
		init_cond_B,	# Model 007
		init_cond_C,	# Model 008
		init_cond_B,	# Model 009
		init_cond_B,	# Model 010
		init_cond_B,	# Model 011
		init_cond_B,	# Model 012
		init_cond_B,	# Model 013
		init_cond_A,	# Model 014
		init_cond_A,	# Model 015
		init_cond_A,	# Model 016
		init_cond_C,	# Model 017
		init_cond_A,	# Model 018
	]

end;

# ╔═╡ 68a93b3c-5aa2-4118-b9f4-a21de21d796d
md"## Parameters"

# ╔═╡ bee70d7a-efa3-4dc8-a3c4-b539b9b4719d
begin

    # From a density of 10 in internal units (details in constants_calculation.jl)
	
	# Models 001, 002, 003, 004, 015
	params_A = [
		n_density,	    # n [cm^(-3)]
		z₀,		    	# Z
	]
...
# ╔═╡ d54ca6b9-f917-48e3-b4a8-119adec185cc
md"# Comparison of the star formation efficiency"

# ╔═╡ 052e0ce3-a559-4852-851c-87165289dd21
md"## Integration"

# ╔═╡ 4781f0f7-ead7-4d3b-911d-d83c8e99a48d
md"## Cleaning and adapting the solutions"

# ╔═╡ 0c7c93f9-b6ce-488e-bdb6-34930608440c
md"# Plots"

# ╔═╡ 38dceb5f-326d-4d78-bb57-b9843efcf8ca
begin
	log_start = -5
	log_end = log10(t_end)
	points = 1e4
...
# ╔═╡ 4ddf6111-7119-4882-a344-6238f3dea940
labels = hcat("model " .* string.(vcat(LOAD_MODELS_A, LOAD_MODELS_C))...);

# ╔═╡ 304e552d-504e-4fc8-ad1f-f99c2888f59f
md"# Auxiliary functions"

# ╔═╡ 5034f29f-4ffb-4a2e-b15f-c32017932263
md"## Numerical integration"

# ╔═╡ 3d6c7708-69f4-46f5-81df-0f800ea02fb0
function integrate_model(ode_system, y₀, tspan, params)
    prob = ODEProblem(ode_system, y₀, tspan, params)
    return solve(prob, Rodas5(), reltol = 1e-6, abstol = 1e-8)
end;

# ╔═╡ 23b54ecb-88a1-4d3c-8003-79441267b549
md"## Include notebooks as modules"

# ╔═╡ 3ab10bcf-fa4f-4fa2-8120-e707b0c6627a
...
# ╔═╡ f3a8008b-195f-4b7b-9870-846a1e0379ab
# Run selected models inside a module 
begin
	models_A = [include_module(path) for path in paths[LOAD_MODELS_A]]
	models_C = [include_module(path) for path in paths[LOAD_MODELS_C]]
end;

# ╔═╡ 224211d9-c03d-472f-bb02-575f64211735
solutions_A = [
	integrate_model(model.system!, init_cond, tspan, param) 
	for (model, init_cond, param) 
	in zip(models_A, initial_conditions[LOAD_MODELS_A], parameters[LOAD_MODELS_A])
];

# ╔═╡ 525be613-6dec-44de-a8ba-6d8165673214
solutions_C = [
	integrate_model(model.system!, init_cond, tspan, param) 
	for (model, init_cond, param) 
	in zip(models_C, initial_conditions[LOAD_MODELS_C], parameters[LOAD_MODELS_C])
];

# ╔═╡ c1537ea5-7c50-4272-a358-50ea0da2e2f9
begin
	
	discrete_sols_A = [solution(times) for solution in solutions_A]
	discrete_sols_C = [solution(times) for solution in solutions_C]
	
	safe_t = [
		replace(x -> x <= 0 ? NaN : x, discrete_sol.t) 
		for discrete_sol in vcat(discrete_sols_A, discrete_sols_C)
	]
	
	safe_s_A = [
		replace(x -> x <= 0 ? NaN : x, discrete_sol[5, :]) 
		for discrete_sol in discrete_sols_A
	]

	safe_s_C = [
		replace(x -> x <= 0 ? NaN : x, (discrete_sol[5, :] ./ density_3)) 
		for discrete_sol in discrete_sols_C
	]
	
	safe_s = vcat(safe_s_A, safe_s_C)
	
end;

# ╔═╡ dfea0dcc-9ec9-4546-a96d-a6e96f13f2c2
plot(
	hcat(safe_t...), 
	hcat(safe_s...), 
	xaxis = ("t / Gyr", :log), 
	yaxis = ("stellar fraction", :log),
	label = labels,
	legend = :bottomright,
	legendfontsize = 10,
	guidefont = 16, 
	...
	linewidth = 2, 
	linestyle = :auto,
	seriescolor = [:red :blue :green :pink :purple :orange :black :cyan :brown :gray :magenta],
)

# ╔═╡ be29b9bd-a831-4763-b092-5143d2f3fb27
plot(
	hcat(safe_t[[2, 10, 11, 9, 7, 8]]...), 
	hcat(safe_s[[2, 10, 11, 9, 7, 8]]...), 
	xaxis = ("t / Gyr", :log), 
	yaxis = ("stellar fraction", :log),
	label = ["Model A" "Model C" "Model D" "Model E" "Model F" "Model G"],
	legend = :bottomright,
	legendfontsize = 10,
	guidefont = 16, 
	tickfont = 12, 
	...
	linewidth = 2, 
	linestyle = :auto,
	seriescolor = [:red :blue :green :cyan :purple :orange],
)
...
git-tree-sha1 = "b9b8bbff3d08e2e919a0f568d24ef0a1e1eaab62"

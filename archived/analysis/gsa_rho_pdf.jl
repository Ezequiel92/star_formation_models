...
using OrdinaryDiffEq, PlutoUI, SpecialFunctions, QuadGK, GlobalSensitivity, CairoMakie

# ╔═╡ a4bc5fbc-a038-4446-95b7-efd0cc5c06ae
TableOfContents(title="🌌 Analysis of model 019", depth=4)
...
# ╔═╡ 736215cb-84ad-42f4-8bcf-e598af17ef8b
begin
	const NUMEQU = 5    # Number of equations
	const NUMPAR = 1    # Number of parameters
end;

# ╔═╡ 2d7585d7-1884-4625-ab69-fe4627b640a7
# Constants for the model 019
begin  
	C1 = 3.03e-6      	# [Gyr]
	C2 = 8.75e-5        # [Gyr]
	K = 810e-3  		# [Gyr Mₒ^(1/2) pc^(-3/2)]
...
# Reference values for the density PDFs
param = Params(
	b = 0.5,
	Ms = 10.0,
	α = 2.0,
...
# ╔═╡ 3ad3e6de-d54d-4da3-b5ca-baeb5991d7df
md"""
### System of equations

> Each equation has $\text{Gyr}^{-1}$ as its units on the LHS and RHS
...
# ╔═╡ 877fdf01-216e-423d-bb71-11cf4e876736
...
# ╔═╡ a313a0b3-f000-4b48-9f57-10fda58eecf1
...
function star_fraction(ρ_PDF, params; sfr = false, init_cond = f₀)
	...
	if ρ_PDF === nothing
		...
		########################################################
		# Standar model 019, without the use of the density PDF
		########################################################
		
		sol = integrate_model(init_cond, tspan, params.ρ₀)
		inter_sol = sol(times)
		...
		for i in 1:params.divisions
        	...
        	sol = integrate_model(init_cond, tspan, params.ρ₀ * exp(s))
			inter_sol = sol(times)
...
let
	...
	lines!(ax, x_axis, Burkhart2018, linewidth = 3, label = "Burkhart2018")
	axislegend(;position = :rt)

	f
end

# ╔═╡ 7e1e723b-d228-4099-8905-bd811f4c3274
md"# Global sensitivity"

# ╔═╡ e2e33fde-7354-456a-976b-65b259d5e585
GS_Burkhart2018 = function (par)
	_, sf = star_fraction(
		pBurkhart2018, 
		Params(
			b = par[1],
...
# ╔═╡ 5a9354a3-be7f-4f15-b1fb-e9a7180ba7be
bounds = [[0.1, 1.5], [1, 30], [0.5, 3]]; # b, Ms, α

# ╔═╡ 92e1ff80-8810-49fb-90d3-b4fb1d309c90
gsa_efast = gsa(GS_Burkhart2018, eFAST(), bounds)

# ╔═╡ d9127c49-0a8b-4eaa-967a-ad9107156c30
let
	f = Figure()
	barplot(
		f[1,1], 

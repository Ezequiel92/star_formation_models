...
TableOfContents(title="🌌 Model 013", depth=4)
...
Markdown.MD(
	Markdown.Admonition(
		"note", 
		"Names", 
		[md"Model 013", md"Full model (Kennicutt-Schmidt law without infall and outflow)"]
	)
)
...
> Each equation has $\text{M}_\odot\,\text{pc}^{-2}\,\text{Gyr}^{-1}$ as its units on the LHS and RHS
...
plot(
	...
	safe_i, 
	xaxis = ("t / Gyr", :log), 
	yaxis = L"ionized gas density / $\textrm{M}_\odot\,\textrm{pc}^{-2}$",
)
...
plot(
	...
	safe_a, 
	xaxis = ("t / Gyr", :log), 
	yaxis = L"atomic gas density / $\textrm{M}_\odot\,\textrm{pc}^{-2}$",
)
...
plot(
	...
	safe_m, 
	xaxis = ("t / Gyr", :log), 
	yaxis = L"molecular gas density / $\textrm{M}_\odot\,\textrm{pc}^{-2}$",
)
...
plot(
	...
	safe_z, 
	xaxis = ("t / Gyr", :log), 
	yaxis = L"metal density / $\textrm{M}_\odot\,\textrm{pc}^{-2}$",
)
...
plot(
	...
	xaxis = ("t / Gyr", :log),
	yaxis = (
		L"stellar density / $\textrm{M}_\odot\,\textrm{pc}^{-2}$", 
		:log,
	),
)

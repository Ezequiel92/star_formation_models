...
using OrdinaryDiffEq, Plots, Symbolics, LinearAlgebra, PlutoUI, LaTeXStrings, PGFPlotsX

# ╔═╡ a4bc5fbc-a038-4446-95b7-efd0cc5c06ae
TableOfContents(title="🌌 Model 008", depth=4)

# ╔═╡ ad37ad37-76c8-4bbe-9175-4b0a5de9be2e
md"##### Initializing packages"

# ╔═╡ 2f6e0de0-90c9-11eb-0c0a-4f669dbc8bc4
md"# Model description"

# ╔═╡ d612d3dd-2410-4784-8106-21b4e63bcf87
Markdown.MD(
	Markdown.Admonition(
		"note", 
		"Names", 
		[md"Model 008", md"Model C", md"Self consistent Ascasibar et al. model (constant $\tau_S$)"]
	)
)

# ╔═╡ 28266da2-90cb-11eb-3ee7-23d1793a9c1e
md"## Constants"

# ╔═╡ 268c3da4-3c0e-4a30-ae67-f9ab2d2de5e0
begin
	const NUMEQU = 5    # Number of equations
	const NUMPAR = 0    # Number of parameters
end;

# ╔═╡ ff02cfde-90ca-11eb-32a9-cbe9a6eef2f5
begin
	C1 = 3.03e-6      		# [Gyr]
	C2 = 8.74e-5            # [Gyr]
	τS = 2.6                # [Gyr]
...
# ╔═╡ f46e607e-90ca-11eb-380d-e30d8ef02b56
md"""
## System of equations

> Each equation has $\text{M}_\odot\,\text{pc}^{-3}\,\text{Gyr}^{-1}$ as its units on the LHS and RHS
...
# ╔═╡ d48128c0-90ca-11eb-36b0-d12ef4744f11
...
# ╔═╡ bbbf2f72-d888-4b73-bc7d-033143d67b88
md"## Stiffness at the initial condition"

# ╔═╡ d0954890-90ca-11eb-2e8f-c13613f67d32
md"# Solving the system"

# ╔═╡ 97a7a7c0-90cb-11eb-1143-8994c376ceff
md"## Integration span"

# ╔═╡ a00735c0-90cb-11eb-127c-7bf87fd0d7d3
begin
	t_start = 0.0    # [Gyr]
	t_end = 1.0      # [Gyr]           
	tspan = (t_start, t_end)
...
# ╔═╡ a9d9dbc2-90cb-11eb-3889-579b0a87c357
md"## Initial conditions"

# ╔═╡ eb784260-90cb-11eb-1a5e-53c8cfdfbdec
begin
...
# ╔═╡ 08758620-90cc-11eb-1813-27747870d34e
md"## Parameters"

# ╔═╡ 3da5180e-90cc-11eb-29d5-ab4abf9bd986
md"There are no parameters in this model."

# ╔═╡ fed94c0a-b042-43a1-9513-63a3694bb67f
params = nothing

# ╔═╡ 1726d9b8-36eb-48c4-bc94-cc311c414079
md"## Integration"

# ╔═╡ 09d1599b-eac9-4e9f-9d8a-5d8307c589fe
md"#### Integration statistics"

# ╔═╡ 9f6088f9-b4ed-4044-b377-9c517a7be870
md"# Plots"

# ╔═╡ fc73dfe5-ec49-467e-af57-fde66db0dde2
begin
	default(
		guidefont = 16, 
		tickfont = 14, 
...
# ╔═╡ 8e53b695-0625-4ceb-a7cb-038f75a01ad4
begin
	log_start = -5
	log_end = log10(t_end)
	points = 1e4
...
# ╔═╡ f4f0d650-4e8b-43c5-999c-219a35cc1e7e
md"# Auxiliary functions"

# ╔═╡ 42fa41b0-90cb-11eb-1e63-15a9e00191d0
md"## Numerical integration"

# ╔═╡ 50ed1e4e-90cb-11eb-3848-15a000477f6c
function integrate_model(y₀, tspan, params)
    prob = ODEProblem(system!, y₀, tspan, params)
    return solve(prob, Rodas5(), reltol = 1e-6, abstol = 1e-8)
end;

# ╔═╡ b1477940-d551-451f-9407-49f13dcd4b11
sol = integrate_model(init_cond, tspan, params)

# ╔═╡ 75cbb775-a9a7-434d-90e9-5c1071b8a74c
sol.destats

# ╔═╡ 3c500211-c639-4cb1-960a-e096b402fa16
begin
	inter_sol = sol(times)
	safe_t = replace(x -> x <= 0 ? NaN : x, inter_sol.t)
	safe_i = replace(x -> x <= 0 ? NaN : x, inter_sol[1, :])
	safe_a = replace(x -> x <= 0 ? NaN : x, inter_sol[2, :])
	safe_m = replace(x -> x <= 0 ? NaN : x, inter_sol[3, :])
	safe_z = replace(x -> x <= 0 ? NaN : x, inter_sol[4, :])
	safe_s = replace(x -> x <= 0 ? NaN : x, inter_sol[5, :])
end;

# ╔═╡ 008fe112-08d0-4f11-bbcf-5a24e596c431
plot(
	safe_t, 
	safe_i, 
	xaxis = ("t / Gyr", :log), 
	yaxis = (
		L"ionized gas density / $\textrm{M}_\odot\,\textrm{pc}^{-3}$", 
		:log,
	),
)

# ╔═╡ 7ed697cc-01d6-4405-9320-a05616a03c29
plot(
	safe_t, 
	safe_a, 
	xaxis = ("t / Gyr", :log), 
	yaxis = L"atomic gas density / $\textrm{M}_\odot\,\textrm{pc}^{-3}$",
)

# ╔═╡ 118299f8-98e8-4f28-844d-e2394ccd276a
plot(
	safe_t, 
	safe_m, 
	xaxis = ("t / Gyr", :log), 
	yaxis = (
		L"molecular gas density / $\textrm{M}_\odot\,\textrm{pc}^{-3}$", 
		:log,
	),
)

# ╔═╡ 2e5ef3a8-ed1f-453d-9374-29393dad9f4e
plot(
	safe_t, 
	safe_z, 
	xaxis = ("t / Gyr", :log), 
	yaxis = L"metal density / $\textrm{M}_\odot\,\textrm{pc}^{-3}$",
)

# ╔═╡ ddc9628e-eb9d-4584-8652-efb6f344a588
plot(
	safe_t, 
	safe_s, 
	xaxis = ("t / Gyr", :log),
	yaxis = (
		L"stellar density / $\textrm{M}_\odot\,\textrm{pc}^{-3}$", 
		:log,
	),
)

# ╔═╡ e752a223-52a9-4351-b624-8da1027151e8
md"## Stiffness coefficient"

# ╔═╡ 636f5c2c-0264-4c78-8554-6c7ede92214e
function stiffness_coeff(eqs, y₀, params, t₀)
	
	@variables t y[1:NUMEQU] par[1:NUMPAR]
	dydt = Vector{Num}(undef, NUMEQU)
	eqs(dydt, y, par, t)
	
	# Symbolic jacobian as an Array
	jac = Symbolics.jacobian(dydt, y, simplify = true)
	
	# Numerical Jacobian as an Array
	eval_jac = [
		build_function(
			jac[i,j], y, par, t, expression = Val{false},
		)(y₀, params, t₀) 
		for i in 1:NUMEQU 
		for j in 1:NUMEQU
		]
	
	# Numerical Jacobian as a Matrix
	jac_mat = reshape(eval_jac, (NUMEQU, NUMEQU))
	
	# Norm of the real part of the non-zero eigenvalues
	eigen_vals = filter(x -> x > eps(typeof(x)), eigvals(jac_mat) .|> real .|> abs)
	
	# Stiffness coefficient
	return maximum(eigen_vals) / minimum(eigen_vals)
	
end;

# ╔═╡ 66e84d45-6859-4b0e-88fb-ed2c5aab826d
stiffness_coeff(system!, init_cond, params, t_start)

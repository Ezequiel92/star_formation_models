...
static const char NAME[] = "Model 003"; // Model name
...
int jacobian(double t, const double y[], double *dfdy, double dfdt[], void *params)
{
    ...
    gsl_matrix_set(m, 0, 2, 367.41923076923075);
    gsl_matrix_set(m, 0, 3, 0);
    gsl_matrix_set(m, 0, 4, 0);

    gsl_matrix_set(m, 1, 0, 8158. * n);
    gsl_matrix_set(m, 1, 1, -283.5820896 * n * (0.0000134 + y[3]));
    gsl_matrix_set(m, 1, 2, -220.90769230769226);
    gsl_matrix_set(m, 1, 3, -283.5820896 * n * y[1]);
    gsl_matrix_set(m, 1, 4, 0);

    gsl_matrix_set(m, 2, 0, 0);
    gsl_matrix_set(m, 2, 1, 283.5820896 * n * (0.0000134 + y[3]));
    gsl_matrix_set(m, 2, 2, -146.89615384615382);
    gsl_matrix_set(m, 2, 3, 283.5820896 * n * y[1]);
    gsl_matrix_set(m, 2, 4, 0);

    gsl_matrix_set(m, 3, 0, 0);
    gsl_matrix_set(m, 3, 1, 0);
    gsl_matrix_set(m, 3, 2, 0.3846153846153846 * (0.0162 - 1. * y[3]));
    gsl_matrix_set(m, 3, 3, -0.3846153846153846 * y[2]);
    gsl_matrix_set(m, 3, 4, 0);

    gsl_matrix_set(m, 4, 0, 0);
    gsl_matrix_set(m, 4, 1, 0);
    gsl_matrix_set(m, 4, 2, 0.3846153846153846);

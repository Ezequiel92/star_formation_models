...
* Model 006
*
* Alternative names: 'Modified basic model with three changes'
*******************************************************************************************/

const char NAME[] = "Model 006"; // Model name
...
const double g = 1e-3 * 14.8285; // [Mₒ pc^(-2)]
...
struct jacobian
{
    ...
    void operator()(const State &y, Matrix &J, const double &t, State &dfdt)
    {
        ...
        J(1, 1) = -2.790447761 * pow(g, 2) * (0.0000134 + y[3]) * (1. + y[3]);
        J(1, 2) = -220.9076923;
        J(1, 3) = 13.509648 * pow(g, 2) * y[0] - 2.790447761 * pow(g, 2) * y[1] * (0.0000134 + y[3]) - 2.790447761 * pow(g, 2) * y[1] * (1. + y[3]);
        J(1, 4) = 0;

        J(2, 0) = 0;
        J(2, 1) = 2.790447761 * pow(g, 2) * (0.0000134 + y[3]) * (1. + y[3]);
        J(2, 2) = -146.8961538;
        J(2, 3) = 2.790447761 * pow(g, 2) * y[1] * (0.0000134 + y[3]) + 2.790447761 * pow(g, 2) * y[1] * (1. + y[3]);
        J(2, 4) = 0;

        J(3, 0) = 0;
        J(3, 1) = 0;
        J(3, 2) = 0.3846153846 * (0.0162 - 1. * y[3]);
        J(3, 3) = -0.3846153846 * y[2];
        J(3, 4) = 0;
